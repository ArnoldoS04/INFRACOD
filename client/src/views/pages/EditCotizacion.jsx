...
const EditCoti = () => {
  ...
  const generatePDF = async () => {
    try {
      const res = await authFetch.put(`/sales/dataedit/${correlativo}`, {
  ...
  const handleObtieneDatos = async (correlativo) => {
    try {
      ...
      const fechaFormateada = res.data.coti_fecha_valida?.slice(0, 10); // "2025-08-03"
      // Asignar datos a estados
      setEmpresa(res.data.coti_empresa || "");
      setNombreCliente(res.data.coti_nombre || "");
      setNit(res.data.coti_nit || "");
      setDireccion(res.data.coti_direccion || "");
      setContacto(res.data.coti_tel || "");
      setNog(res.data.coti_nog || "");
      setFechaValida(fechaFormateada || "");

      // Si la respuesta trae items, asignarlos y mostrar tabla
      if (
        res.data.detalle_cotizacion &&
        Array.isArray(res.data.detalle_cotizacion) &&
        res.data.detalle_cotizacion.length > 0
      ) {
        // Mapeamos cada ítem agregando su total calculado
        const detalleConTotales = res.data.detalle_cotizacion.map((item) => ({
          ...item,
          total:
            parseFloat(item.det_cantidad) *
            parseFloat(item.det_precio_unitario),
        }));
        setItems(detalleConTotales);
        setShowTable(true);
      } else {
        setItems([]);
        setShowTable(false);
